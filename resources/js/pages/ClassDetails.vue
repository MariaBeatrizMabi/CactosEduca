...
async function fetchAvailableTeachers() {
    ...
    const { data: school } = await api.get(`/api/users/${teacher.teacher.school_id}/management-schools`);
    const { data } = await api.get(`/api/management-schools/${school.management_school.id}/teachers`);
